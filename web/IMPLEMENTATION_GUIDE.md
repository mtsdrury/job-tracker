...
- **AI-only message drafting:** Replaced with tiered approach — free users get template-based drafting, Pro users get AI rewrite via Claude Haiku.

### Completed Since Initial Build

- **Stripe Billing (Phase C):** stripe.ts singleton, checkout/webhook/portal/prices API routes, full billing page with 3 pricing tiers ($12/mo, $29/3mo, $49/6mo), Stripe test mode configured
- **Message Drafting System:** AI draft-message API route (Pro-gated), template-substitution utility, inline drafting panel on outreach cards (template selector, connection selector, copy, save draft, AI rewrite)
- **Next Action Derivation (Phase D):** `src/lib/next-action.ts` rules engine with full lifecycle coverage (pre-apply, post-apply, closed states, strategy-aware, urgency levels). Hooked into all 4 mutation routes (jobs POST/PATCH, outreach POST/PATCH). Dashboard refactored to use centralized `generateNudges()`. Jobs list and detail pages show derived actions with color-coded badges.

### Not Yet Started

- **Phase E: UX Polish** — Loading states, empty states, error boundaries, mobile responsiveness, form validation
- **Phase G: Landing Page** — Update with KnowSomeone branding, $12/mo pricing copy, feature marketing
- **Phase H: Testing & Edge Cases** — Privacy/Terms pages, archived job handling, deleted contact edge cases
- **Phase I: Deployment** — Vercel deploy, env config, Stripe webhook pointing, launch checklist
...
### Phase C: Stripe Billing — DONE
Checkout, webhook, portal, prices API routes. Billing page with 3 tiers. Test mode configured.
